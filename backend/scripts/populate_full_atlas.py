...
async def main():
    ...
    print("\n=== STEP 2.1: Cleaning up dummy / duplicate AU records ===")
    # Delete artificial dummy records generated by previous suffix expansion
    del_res = await db["occupation_master"].delete_many({
        ...
        "$or": [
            ...
            {"title": {"$regex": r"^(Specialist|Senior|Consultant) \w+ \(\d+\)"}},
            {"title": {"$regex": r"\(nec\)$"}, "code": {"$regex": r"(12|13|14|99)$"}, "tasks": {"$size": 4}},
        ]
    })
    print(f"✔ Cleaned {del_res.deleted_count} dummy AU occupations from previous generated runs.")

    # Deduplicate AU occupation_master by code (keeping the most complete document)
    seen_codes = set()
    dup_ids_to_delete = []
    async for occ in db["occupation_master"].find({"country_code": "AU"}).sort([("status", -1), ("updated_at", -1)]):
        code = occ.get("code")
        if not code:
            dup_ids_to_delete.append(occ["_id"])
            continue
    ...
    if dup_ids_to_delete:
        await db["occupation_master"].delete_many({"_id": {"$in": dup_ids_to_delete}})
        print(f"✔ Deduplicated {len(dup_ids_to_delete)} duplicate AU records.")
        ...
        for code, n in by_code.items():
            ...
            if not existing:
                doc = {
                    ...
                    "classification_dual_code": n.get("classification_dual_code") or {"anzsco_v1_3": code, "anzsco_v2022": code, "mapped": True},
                ...
                # Update base Home Affairs metadata if missing
                upd = {}
                if not existing.get("title") and n.get("title"):
                    upd["title"] = n["title"]
                if not existing.get("visa_pathways") and n.get("visa_pathways"):
                    upd["visa_pathways"] = n["visa_pathways"]
                if not existing.get("pathway_list") and n.get("pathway_list"):
                    upd["pathway_list"] = n["pathway_list"]
                if not existing.get("assessing_authority") and n.get("assessing_authority"):
                    upd["assessing_authority"] = n["assessing_authority"]
                if upd:
                    await db["occupation_master"].update_one({"_id": existing["_id"]}, {"$set": upd})
                    updated_ha += 1

        print(f"✔ Home Affairs Ingestion: {inserted_ha} inserted, {updated_ha} updated (Total available: {len(by_code)})")
    except Exception as e:
    ...
    print("\n=== STEP 5: Authority Linkage & 13-Metric Field Enrichment ===")
    ...
    PREFIX_TO_AUTH = {
        ...
        # Architecture & Quantity Surveying -> AACA / ISNSW / SSSI
        ...
        # Trades -> TRA (Major Group 3 & Trades in Group 4)
    ...
    def resolve_auth(occ: Dict[str, Any]) -> Dict[str, Any]:
        # 1. Check existing embedded authority name / short_name
        embedded = occ.get("assessing_authority") or {}
        if isinstance(embedded, dict):
            for k in ["short_name", "code", "name"]:
                val = str(embedded.get(k) or "").strip()
    ...
    async for occ in db["occupation_master"].find({"country_code": "AU"}):
        ...
        # 1. anzsco_profile (Salary & Workforce)
        ...
        # 7. ila_eligibility (Industry Labour Agreement)
        ...
        # 9. skill_assessment_details (Skill Body Criteria)
        ...
        # 10. visa_pathways (Visa Eligibility)
        ...
        # 11. state_territory_eligibility (State Nomination)
        ...
        # 12. skillselect_tier (SkillSelect Tier)
        if is_val_empty(occ.get("skillselect_tier")):
            updates["skillselect_tier"] = "tier_2"

        # 13. anzsco parent links
        if len(code_str) == 6 and code_str.isdigit():
            updates["anzsco_4digit_code"] = code_str[:4]
            updates["anzsco_major_group_code"] = code_str[0]
