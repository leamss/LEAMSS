...
async def get_batch(batch_id: str, current_user: dict = Depends(get_current_user)):
    ...
    valid_count = sum(1 for r in rows if r.get("status") == "valid")
    ...
    if batch.get("generated") != generated_count or batch.get("valid") != valid_count or batch.get("total") != len(rows):
        ...
        batch["needs_ai"] = needs_ai_count
        batch["invalid"] = error_count
        batch["total"] = len(rows)
        await BATCHES.update_one(
            {"id": batch_id},
            {"$set": {"generated": generated_count, "valid": valid_count, "needs_ai": needs_ai_count, "invalid": error_count, "total": len(rows)}}
        )
...
def _render_row_whatsapp_text(row: Dict[str, Any], tmpl_body: str, upload_url: Optional[str] = None) -> str:
    ...
    if improvements_list:
        # Keep top 2 concise steps to guarantee total message fits under 900 chars
        clean_steps = []
        ...
        improvements_text = (
            ...
            "   Superior English (PTE 79+) provides +20 points.\n\n"
            "🔹 *Step 2: State / Regional Nomination (+5 to +15 Pts)*\n"
            "   State nomination adds +5 (190) or +15 (491) points."
        )
...
async def whatsapp_row(row_id: str, req: RowWhatsAppRequest, current_user: dict = Depends(get_current_user)):
    ...
    if req.phone_override:
        set_fields["parsed.phone"] = phone
    await ROWS.update_one({"id": row_id}, {"$set": set_fields})
