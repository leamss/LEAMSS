...
//   };
  ...
  const addSection = () => {
    ...
    onChange([...sections, { id, title: 'New Section', fields: [] }]);
    setExpanded({ ...expanded, [id]: true });
  };

  const updateSection = (idx, field, val) => {
    const s = [...sections]; s[idx][field] = val; onChange(s);
  };

  const removeSection = (idx) => {
    if (!window.confirm('Delete this section and all its fields?')) return;
    const s = [...sections]; s.splice(idx, 1); onChange(s);
  };
  ...
  const updateField = (sIdx, fIdx, field, val) => {
    const s = [...sections]; s[sIdx].fields[fIdx][field] = val; onChange(s);
  };

  const removeField = (sIdx, fIdx) => {
    const s = [...sections]; s[sIdx].fields.splice(fIdx, 1); onChange(s);
  };
                            ...
                            <div key={fIdx} className="p-3 bg-slate-50 rounded-lg border border-slate-200" data-testid={`field-${sIdx}-${fIdx}`}>
                              <div className="grid grid-cols-12 gap-2 items-start">
                                ...
                                <div className="col-span-3 flex items-center flex-wrap gap-2 pb-0.5">
                                  ...
                                  <label className="flex items-center gap-1 cursor-pointer select-none bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded text-amber-800" title="Lock this field for clients until unlocked">
                                    <input type="checkbox" checked={Boolean(field.is_locked_until_paid || field.is_locked)} onChange={e => { updateField(sIdx, fIdx, 'is_locked_until_paid', e.target.checked); updateField(sIdx, fIdx, 'is_locked', e.target.checked); }} className="rounded h-3 w-3 text-amber-600" />
...
export default WorkflowIntakeEditor;
