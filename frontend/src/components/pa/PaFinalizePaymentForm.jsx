...
const sum = schedule.reduce((s, r) => s + (parseFloat(r.amount) || 0), 0);
    ...
    {schedule.map((r, i) => (
        ...
        <button type="button" onClick={() => remove(i)} className="text-rose-500 hover:text-rose-700 text-xs p-1" title="Remove"><X className="h-3.5 w-3.5" /></button>
        </div>
        ))}
    {totalAmount != null && (
...
export default function PaFinalizePaymentForm({
  ...
  requiresPartnerInfo, getAuthHeader, onSpouseSaved,
    ...
    // Render spouse form INSTEAD of payment method UI, until saved
    if (needsSpouseForm) {
    return (
