...
import { FileText, RefreshCw, Eye, Download, XCircle, ClipboardCheck } from 'lucide-react';
 ...
 * Splits documents into "Client Documents" and "Pre-Assessment Report" (admin-uploaded).
 */

// 👇 NEW — shared row renderer used by both sections
function DocRow({ d, pa, getAuthHeader, onRefresh }) {
  ...
  return (
    <div className="flex items-center gap-1.5 text-xs bg-white rounded px-2 py-1.5 border border-slate-100">
...
export default function PaDocumentsList({ pa, docs, onRefresh, getAuthHeader }) {
  ...
  // 👇 NEW — split documents by who uploaded them
  const adminDocs = docs.filter(d => d.uploaded_by_role === 'admin');
  const clientDocs = docs.filter(d => d.uploaded_by_role !== 'admin');
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── Pre-Assessment Report box (admin-uploaded) — only shows if admin has uploaded something ── */}
      {adminDocs.length > 0 && (
        <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-semibold text-emerald-800 flex items-center gap-1">
              <ClipboardCheck className="h-3.5 w-3.5" /> Pre-Assessment Report
            </p>
          </div>
          <div className="space-y-1.5">
            {adminDocs.map(d => (
