...
export default function PayoutQueue() {
  ...
  const toggleAll = () => {
    if (selectedRows.length === filtered.length) setSelected({});
    else {
      const next = {};
      filtered.forEach(r => { next[`${r.pa_id}|${r.allocation_id}`] = true; });
      setSelected(next);
    }
  };
        ...
        {selectedRows.length > 0 && (
          <Card className="p-3 mb-3 bg-indigo-50 border-indigo-200 flex items-center justify-between" data-testid="bulk-bar">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" onClick={bulkApprove} disabled={busy} className="bg-indigo-600 hover:bg-indigo-700" data-testid="bulk-approve-btn">Bulk Approve</Button>
              <Button size="sm" onClick={bulkPay} disabled={busy} className="bg-emerald-600 hover:bg-emerald-700" data-testid="bulk-pay-btn"><CheckCircle className="h-3.5 w-3.5 mr-1" />Bulk Mark Paid</Button>
            </div>
          </Card>
        )}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-100">
                  <tr className="text-[10px] uppercase text-slate-500">
                    <th className="px-3 py-2 text-left"><input type="checkbox" checked={filtered.length > 0 && selectedRows.length === filtered.length} onChange={toggleAll} data-testid="select-all" /></th>
                ...
                <tbody>
                  {filtered.map((r, i) => {
                    ...
                    return (
                      <tr key={k} className={`border-t hover:bg-slate-50 ${isSel ? 'bg-indigo-50/40' : ''}`} data-testid={`row-${i}`}>
                        <td className="px-3 py-2"><input type="checkbox" checked={isSel} onChange={() => toggle(r)} data-testid={`row-check-${i}`} /></td>
